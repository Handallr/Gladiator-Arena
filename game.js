...
let enemyHP  = 100;
let score    = 0;
let dir      = 0; // -1 left, +1 right, 0 stop
let isJump   = false;
let isBlock  = false;
let isAttack = false;
...
enemy.className  = 'sprite idle';
player.style.left = '100px';
enemy.style.left  = '736px';

// Prevent scrolling con le frecce
...
window.addEventListener('keyup', e => {
  ...
  if (e.key === 's' || e.key === 'ArrowDown')   stopBlock();
});
...
function attack() {
  ...
  setTimeout(() => {
    isAttack = false;
    player.className = 'sprite idle';
    // logica colpi al nemico se in range...
  }, 200);
}
function block() {
...
// TODO: aggiungi logica nemico, collisioni, HP, score...
